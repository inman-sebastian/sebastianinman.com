/**
 * Shared types + parsing for the rotating hero example cards.
 * Server-safe (no "use client") so content loaders and pages can use it;
 * the animated renderer lives in components/HeroCards.tsx.
 */
...
export type HeroCardIcon = keyof typeof heroCardIcons;
...
export function parseHeroCards(raw: unknown): HeroCard[] | undefined {
  ...
  return raw.map((item) => {
    ...
    const icon = String(entry.icon ?? "check");
    return {
      type: "notice" as const,
      icon: (icon in heroCardIcons ? icon : "check") as HeroCardIcon,
